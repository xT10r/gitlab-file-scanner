...
func TestMaskToFileRegex(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Description, func(t *testing.T) {
			...
			re, err := text.MaskToFileRegex(tc.Mask)
			if err != nil {
				t.Fatalf("Failed to compile regex for mask '%s': %v", tc.Mask, err)
			}
...
func TestSplitMask(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.Description, func(t *testing.T) {
			...
			result := text.SplitMask(tc.Mask)
			if len(result) != len(tc.Expected) {
				t.Fatalf("Expected %d parts, got %d: %v", len(tc.Expected), len(result), result)
			}
...
func TestPluralize(t *testing.T) {
	testCases := []struct {
		...
		{Description: "0 секунд", Count: 0, Expected: ""},
	...
	for _, tc := range testCases {
		t.Run(tc.Description, func(t *testing.T) {
			...
			result := text.GetDurationString(time.Duration(tc.Count) * time.Second)
